  ...
  return (
    <section id="portfolio" className="section-container">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map(({ icon: Icon, title, desc, link }) => (
          <div
            key={title}
            onClick={() => link && navigate(link)}
            className={`glass-card p-8 group hover:glow-border transition-all duration-300 hover:-translate-y-1 ${link ? "cursor-pointer" : ""}`}
          >
